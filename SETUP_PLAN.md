...
This web application is a **client-side SPA only** and relies on a separate Node.js application to:
- Provide the REST API backend
- Trigger the build process
- Serve the built SPA in production

**Important**: This repository contains **no server functionality**. However, it includes a standalone build process for CI/CD purposes that outputs static files to be served by the separate Node.js application.
...
# Build output (generated by CI and for production)
dist/
...
### Step 1.3: Initialize Package.json
```bash
npm init -y
```
...
# Build the project (standalone build for CI/production)
npm run build
# This creates a 'dist' folder with static files to be served by the separate Node.js application

# Start development server (for local development only)
npm run dev
```
